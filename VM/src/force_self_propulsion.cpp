...
{
  Vec ForceSelfPropulsion::compute(const Vertex<Property>& v, const HalfEge<Property>& he)
  {
    
    Face<Property>& f   = *(he->face());         // cell to the right of the half edge
    
    double v0 = (f.outer)   ? 0.0 : _v0[fh->data().face_type];
    Vec  n  = (f.outer)   ? Vec(0,0) : _n[fh->data().face_type];
